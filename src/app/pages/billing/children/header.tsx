"use client";

export default function InvoiceHeader() {
  function createInputFileElement() {
    const body = document.body;
    const input = document.createElement("input");
    input.type = "file";
    input.accept = "image/*";
    input.id = "file";
    input.addEventListener("change", (evt) => {
      console.log("evt: ", evt);
      input.remove();
    });

    body.appendChild(input);
    input.click();
  }

  return (
    <div>
      <div className="flex justify-between">
        <div className="text-2xl font-bold text-sky-600">
          FATURA SETEMIL SOLUÇÕES
        </div>
        <div className="flex gap-5">
          <button className="w-8 h-8 flex justify-center items-center rounded-full bg-sky-100 hover:bg-sky-200">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-6 h-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0021 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 00-1.913-.247M6.34 18H5.25A2.25 2.25 0 013 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 011.913-.247m10.5 0a48.536 48.536 0 00-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18 10.5h.008v.008H18V10.5zm-3 0h.008v.008H15V10.5z"
              />
            </svg>
          </button>
          <button className="w-8 h-8 flex justify-center items-center rounded-full bg-sky-100 hover:bg-sky-200">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-6 h-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99"
              />
            </svg>
          </button>
        </div>
      </div>
      <div className="mt-12 flex justify-between items-center">
        <div className="flex flex-col gap-3">
          <div className="flex">
            <p className="font-medium">FATURA Nº</p>
            <input
              type="text"
              className="ml-[6rem] h-8 w-64 border-0 pl-3 ring-1 ring-gray-300 cursor-pointer"
              placeholder="Nº da Fatura"
            />
          </div>
          <div className="flex">
            <p className="font-medium">DATA DA FATURA</p>
            <input
              type="date"
              className="ml-[3rem] h-8 w-64 px-3 border-0 ring-1 ring-gray-300 cursor-pointer"
            />
          </div>
        </div>
        <div
          onClick={createInputFileElement}
          className="bg-sky-100 w-32 h-32 cursor-pointer flex justify-center items-center hover:bg-sky-200"
        >
          <div className="bg-white p-2 rounded-sm">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-6 h-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"
              />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}
